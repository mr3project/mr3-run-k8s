...
export function check(input: OptionalFields<T>): CheckResult[] {
  ...
  if (service.useHttps) {
    checkNonEmpty(accum, service.sslCertificateArn, 'sslCertificateArn', "SSL Certificate ARN");
    if (service.sslCertificateArn !== undefined) {
      accumAssert(accum,
        service.sslCertificateArn.trim().startsWith("arn:aws:acm::") &&
        service.sslCertificateArn.trim().includes(":certificate/"),
        "SSL Certificate ARN should be properly set.", 'sslCertificateArn');
    }
  }
