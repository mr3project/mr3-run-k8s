...
import { checkServer } from '../../aws-run-env';
...
export function check(
  ...
  if (eksInput !== null && serviceInput !== null) {
    try {
      // if calling eks/service.validate() fails with AssertionError,
      // do not call checkServer() because we are not ready
      const eksConf: eks.T = eks.validate(eksInput);
      const serviceConf: service.T = service.validate(serviceInput);

      // do not call apps.validate() because check() may be called from validate()
      const appsConf: T = { ...appsInput };
      trimString(appsConf);

      const result = checkServer(eksConf, serviceConf, appsConf);
...
export function validate(input: T, eksConf: eks.T, serviceConf: service.T): T {
  const checkResult = check(input, eksConf, serviceConf);
