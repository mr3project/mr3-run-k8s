...
export function getRunEnvGcp(
    gcpConf: cluster.T,
    serviceConf: service.T,
    appsConf: apps.T): RunEnv {
  const runEnv = getRunEnvForCloudCommon(gcpConf, serviceConf, appsConf);

  runEnv.basicsEnv.externalIp = "";  // use Ingress
...
export function checkServer(
  ...
  const runEnv = getRunEnvGcp(gcpConf, serviceConf, appsConf);
